...
def _get_location_id() -> str:
    ...
    There is deliberately no default: falling back to a hard-coded location
    would silently point the CLI at somebody else's sub-account.
    """
    loc = os.environ.get("GHL_LOCATION_ID", "").strip()
    if not loc:
        print(
            "GHL_LOCATION_ID is not set. Add it to your .env "
            "(the long ID in your GHL URL).",
            file=sys.stderr,
        )
        sys.exit(1)
    return loc
