...
def _get_url() -> str:
    """Get the Nextcloud base URL from the NEXTCLOUD_URL env var.

    There is deliberately no default: falling back to a hard-coded host
    would send credentials to somebody else's server.
    """
    url = os.environ.get("NEXTCLOUD_URL", "").strip()
    if not url:
        print(
            "NEXTCLOUD_URL is not set. Add it to your .env "
